	...
	function sbscrbr_subscribe_form() {
        ...
        if ( empty( $report_message ) ) {
            if ( ! empty( $sbscrbr_options['form_label'] ) ) {
                $content .= '<h3>' . $sbscrbr_options['form_label'] . '</h3>';
            }
        } else {
            $content .= $report_message;
        }
		$content  .= '<form method="post" id="newsletter" action="' .  $page_url . '" class="subscrbr-sign-up-form">';
		$content .= '
		    <label class="email">
			<p>
				<input type="email" name="sbscrbr_email" value="" placeholder="' . $sbscrbr_options['form_placeholder'] . '"/>
			</p>
			</label>';
...
//			</label>';
        ...
        $content.='<label class="sbscrbr-submit-block">
       <p>
